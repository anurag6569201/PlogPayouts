...
def build_model():
    ...
    efficientnetb7_layer = tf.keras.applications.efficientnet.EfficientNetB7(include_top = False, input_shape = (320, 320,3))

 
    for layer in efficientnetb7_layer.layers:

        layer.trainable = False


    efficientnetb7 = Sequential()
    efficientnetb7.add(keras.Input(shape=(320, 320, 3)))

 
    def efficientnetb7_preprocessing(img):
        return tf.keras.applications.efficientnet.preprocess_input(img)

    efficientnetb7.add(Lambda(efficientnetb7_preprocessing))

    efficientnetb7.add(efficientnetb7_layer)
    efficientnetb7.add(tf.keras.layers.GlobalAveragePooling2D())
    efficientnetb7.add(Dense(12, activation='softmax')) 
...
efficientnetb7 = build_model()
efficientnetb7.load_weights('effciientnet_b7_model_weights_updated.weights.h5')
    
//model here
def identifyImage_garbage(file_path):
    
    dict_decode = {0: 'battery', 1: 'biological', 2: 'brown-glass', 3: 'cardboard', 4: 'clothes', 5: 'green-glass', 6: 'metal', 7: 'paper', 8: 'plastic', 9: 'shoes', 10: 'trash', 11: 'white-glass'}
    
    # print(file_path)
    img = tf.keras.utils.load_img(file_path, target_size=(320, 320))
    img_array = tf.keras.utils.img_to_array(img)
    expanded_img = np.expand_dims(img_array,axis=0)
    # preprocessed_img = tf.keras.applications.efficientnet.preprocess_input(expanded_img)
  
    print(efficientnetb7.summary())
    result = efficientnetb7.predict(expanded_img, verbose=0)
...
#     image = load_img(img_path, target_size=(600, 600, 3))
#     img_array = img_to_array(image)
#     img_array = keras.applications.efficientnet.preprocess_input(img_array)
#     img_array = np.expand_dims(img_array, axis=0)
#     model = tf.keras.models.load_model('models/garbage_classification_efficientnetb7_model.keras')
